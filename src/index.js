...
const columns = Math.floor(matrixImgs.length / rows);
let rowElement, gridElement;
matrixImgs.forEach(function(img, i) {
	...
	img.className = 'matrix-item fill-contain';
	img.dataset.pos = i;
...
const showPopup = function(event) {
	...
	const removePopup = function(event) {
		...
		for (
			...
			document
				.querySelectorAll('.secondary-img')[i].removeEventListener('click', swapImg, true);
		}
