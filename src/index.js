...
const createImgElement = function (imgSrc, title) {
  ...
  img.src = new URL(`./${imgFolder}-small/${imgSrc}`, import.meta.url).href;
  img.alt = title;
  ...
  img.dataset.bigSrc = new URL(
    `./${imgFolder}/${imgSrc}`,
    import.meta.url
  ).href;
  img.dataset.smallSrc = new URL(
    `./${imgFolder}-small/${imgSrc}`,
    import.meta.url
  ).href;
  return img;
};
...
const matrixImgs = shuffle(imgData).map((d) =>
  createImgElement(d.imgName, d.title)
);
...
const showPopup = function (event) {
  ...
  if (storyPopup.classList.contains("visible")) {
    return;
  }
  ...
  const removePopup = function (event) {
    ...
    if (event) {
      ...
      if (
        path.includes(storyPopup) &&
        !path.includes(document.querySelector("#close-button"))
      ) {
        return;
      }
    }
    ...
    for (
      let i = 0;
      i < document.querySelectorAll(".secondary-img").length;
      i++
    ) {
      document
        .querySelectorAll(".secondary-img")
        [i].removeEventListener("click", swapImg, true);
    }
    document.querySelector("body").removeEventListener("click", removePopup);
    document
      .querySelector("#close-button")
      .removeEventListener("click", removePopup);
    document
      .querySelector("#prev-button")
      .removeEventListener("click", swapStory);
    document
    ...
    if (!event) {
      timedPopup = window.setTimeout(showPopup, timeBetweenPopup);
    }
  ...
  for (let i = 0; i < document.querySelectorAll(".secondary-img").length; i++) {
    document
      .querySelectorAll(".secondary-img")
      [i].addEventListener("click", swapImg, true);
  }
};

// Attach an event listener to each matrix img to show popup on click
matrixImgs.forEach((d) => d.addEventListener("click", showPopup));
...
if (window.innerHeight < minLargeHeight && !isVisWall) {
  ...
  if (window.innerWidth <= 425) {
    ...
    // Larger than tablet (laptop, desktop, etc.)
    zoomSpeed = 6000;
  }
}
